...
def dqn_learing(
    ...
    for t in count():
        ...
        recent_frame = replay_buffer.encode_recent_observation()
        if t > learning_starts:
            action = select_epilson_greedy_action(Q, recent_frame, t)
        else:
        ...
        if (t > learning_starts and
            ...
            # Normalize and bitwise-not the mask in order to set q_values of "game over" turns to zero when multiplying.
            done_mask[done_mask > 0] = 1
            done_mask[done_mask <= 0] = 0
            done_mask = Variable(~torch.from_numpy(done_mask).byte()).type(dtype)
